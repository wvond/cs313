...
function UpdateCart()
{
	...
	if(cart_products.length>0)
	{
		for(i=0;i<cart_products.length;i++)
		{
			...
			cart_html += '<span class="pull-right">'+currency_symbol+(product_prices[cart_products[i]]*cart_quantities[cart_products[i]]).toLocaleString('en-US', {minimumFractionDigits: 2})+'</span>';
			cart_html += '<hr/>';
			subtotal+=product_prices[cart_products[i]]*cart_quantities[cart_products[i]];
		}
		
		cart_html += '<span class="pull-right">Subtotal: '+currency_symbol+subtotal.toLocaleString('en-US', {minimumFractionDigits: 2})+'</span>';
